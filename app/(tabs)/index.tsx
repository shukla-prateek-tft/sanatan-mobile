...
const FestivalCard = ({ fest }: { fest: FestivalObj }) => {
  ...
  return (
    <View style={[st.festCard, { borderLeftColor: accent }]}>
      <View style={st.festTop}>
        <Text style={st.festEmoji}>{emoji}</Text>
        <View style={{ flex: 1 }}>
          <Text style={[st.festName, { color: accent }]}>{fest.name}</Text>
          {fest.category ? (
            <Text style={st.festMeta}>
      ...
      {fest.description ? (
        <Text style={st.festDesc}>{fest.description}</Text>
      ) : null}
    </View>
  );
};
...
const BiCell = ({
  ...
  const st = React.useMemo(() => getStyles(colors), [colors]);
  return (
    <View style={[st.biCell, fullWidth && st.biCellFull]}>
      <View style={st.biCellLabelRow}>
        <Text style={st.biCellLabelEn}>{labelEn}</Text>
        <Text style={st.biCellLabelHi}>{labelHi}</Text>
      </View>
      <Text style={[st.biCellValueEn, accent && st.accent]}>{valueEn}</Text>
      <Text style={st.biCellValueHi}>{valueHi}</Text>
      {subValue ? <Text style={st.biCellSub}>{subValue}</Text> : null}
    </View>
  );
};
...
const InfoCell = ({
  ...
  return (
    <View style={st.infoCell}>
      ...
      <Text style={[st.infoCellValue, accent && st.accent]}>{value}</Text>
      {sub ? <Text style={st.infoCellSub}>{sub}</Text> : null}
    </View>
  );
};
...
const TimingRow = ({
  ...
  return (
    <View style={st.timingRow}>
      ...
      <Text style={st.timingIcon}>{icon}</Text>
      <View style={st.timingLabels}>
        <Text style={st.timingLabelEn}>{labelEn}</Text>
        <Text style={st.timingLabelHi}>{labelHi}</Text>
      </View>
      <Text style={st.timingTime}>
        {slot.start}
        {"\n"}
        {slot.end}
      </Text>
  ...
  return (
    <View style={st.sectionHeader}>
      ...
      <View style={st.sectionPill}>
        <Text style={st.sectionEn}>{en}</Text>
        <Text style={st.sectionHi}>{hi}</Text>
      </View>
      <View style={st.sectionLine} />
    </View>
  );
...
const Pill = ({ text, color }: { text: string; color?: string }) => {
  ...
  const st = React.useMemo(() => getStyles(colors), [colors]);
  return (
    <View style={[st.pill, { borderColor: color ?? colors.gold }]}>
      <Text style={[st.pillText, { color: color ?? colors.gold }]}>{text}</Text>
    </View>
  );
};
...
export default function HomeScreen() {
      ...
      <GradientBackground>
        <View style={st.loadingContainer}>
          ...
          <ActivityIndicator
            ...
            style={{ marginTop: 24 }}
          />
          <Text style={st.loadingEn}>{t("home.loading")}</Text>
        </View>
      </GradientBackground>
    );
  }
  ...
  return (
    <GradientBackground>
      ...
      >
        {/* ── HEADER ── */}
        <Animated.View entering={FadeIn.duration(1000)} style={st.header}>
          <AnimatedDiya />
          <Text style={st.om}>॥ ॐ नमः शिवाय ॥</Text>
          {panchang && (
            <>
              <Text style={st.dateEn}>{panchang.date}</Text>
              <Text style={st.dateHi}>{panchang.date_hi}</Text>
              {/* Show city from panchangService._loc */}
              {panchang.locationCity ? (
                <View style={st.locationPill}>
                  <Text style={st.locationPillTxt}>
                    📍 {panchang.locationCity}
                  </Text>
                </View>
              ) : null}
        ...
        {dailyMantra && (
          <Animated.View entering={FadeInDown.delay(100).duration(700)}>
            <Card
              title={`✨ ${SECTION_LABELS_HI.mantra} · ${t("home.mantra")}`}
            >
              <Text style={st.mantraText}>{dailyMantra.text}</Text>
              <Text style={st.mantraMeaning}>{dailyMantra.meaning}</Text>
        ...
        {panchang && (
          <>
            ...
            <Animated.View entering={FadeInDown.delay(200).duration(700)}>
              <Card
                title={`📅 ${SECTION_LABELS_HI.calendar} · ${t("home.calendar")}`}
              >
                <View style={st.strip}>
                  <View style={st.stripCell}>
                    <Text style={st.stripLabelHi}>{FIELD_LABELS_HI.masa}</Text>
                    <Text style={st.stripLabelEn}>Masa</Text>
                    <Text style={st.stripValueHi}>{panchang.masa_hi}</Text>
                    <Text style={st.stripValueEn}>{panchang.masa}</Text>
                  </View>
                  <View style={st.stripDivider} />
                  <View style={st.stripCell}>
                    <Text style={st.stripLabelHi}>{FIELD_LABELS_HI.ritu}</Text>
                    <Text style={st.stripLabelEn}>Ritu</Text>
                    <Text style={st.stripValueHi}>{panchang.ritu_hi}</Text>
                    <Text style={st.stripValueEn}>{panchang.ritu}</Text>
                  </View>
                  <View style={st.stripDivider} />
                  <View style={st.stripCell}>
                    <Text style={st.stripLabelHi}>{FIELD_LABELS_HI.vara}</Text>
                    <Text style={st.stripLabelEn}>Vara</Text>
                    <Text style={st.stripValueHi}>{panchang.vara_hi}</Text>
                    <Text style={st.stripValueEn}>{panchang.vara}</Text>
                  </View>
                </View>
                <View style={st.samvatRow}>
                  <View style={st.samvatItem}>
                    <Text style={st.samvatLabelHi}>
                      {FIELD_LABELS_HI.vikramSamvat}
                    </Text>
                    <Text style={st.samvatValue}>{panchang.vikramSamvat}</Text>
                  </View>
                  <View style={st.samvatItem}>
                    <Text style={st.samvatLabelHi}>
                      {FIELD_LABELS_HI.shakaSamvat}
                    </Text>
                    <Text style={st.samvatValue}>{panchang.shakaSamvat}</Text>
                  </View>
                </View>
              </Card>
            </Animated.View>
            ...
            <Animated.View entering={FadeInDown.delay(300).duration(700)}>
              <Card
                title={`🪔 ${SECTION_LABELS_HI.panchang} · ${t("home.panchangTitle")}`}
              >
                <View style={st.grid2}>
                  <BiCell
                    labelEn="Tithi"
            ...
            <Animated.View entering={FadeInDown.delay(400).duration(700)}>
              <Card
                title={`☀️ ${SECTION_LABELS_HI.sunMoon} · ${t("home.sunMoon")}`}
              >
                <View style={st.moonRow}>
                  <MoonPhase phase={panchang.moonPhase} size={90} />
                  <View style={st.moonDetails}>
                    <Text style={st.moonLabelHi}>
                      {FIELD_LABELS_HI.moonPhase}
                    </Text>
                    <Text style={st.moonValue}>
                      {Math.round(panchang.moonPhase * 100)}%{" "}
                      {panchang.moonPhase < 0.5
                        ? FIELD_LABELS_HI.waxing
                        : FIELD_LABELS_HI.waning}
                    </Text>
                    <Text style={st.moonLabelHi}>{FIELD_LABELS_HI.raasi}</Text>
                    <Text style={st.moonValue}>
                      {panchang.raasi_hi} · {panchang.raasi}
                    </Text>
                    <Text style={st.moonLabelHi}>{FIELD_LABELS_HI.paksha}</Text>
                    <Text style={st.moonValue}>{panchang.paksha_hi}</Text>
                  </View>
                </View>
        ...
        <Animated.View
          ...
          style={st.footer}
        >
          <Text style={st.footerMain}>🙏 Jai Shri Ram · जय श्री राम 🙏</Text>
          <Text style={st.footerSub}>॥ सर्वे भवन्तु सुखिनः ॥</Text>
        </Animated.View>
      </ScrollView>
    </GradientBackground>
  );
